...
$sql = "SELECT sd.sensorID AS id, s.sensorName, s.sensorImage, s.sensorColor, s.favorite, sd.sensorValue, s.dataType, sd.time FROM (SELECT sensorID, sensorValue, time FROM a21iot02.sensorData AS b INNER JOIN (SELECT MAX(ID) AS id FROM a21iot02.sensorData GROUP BY sensorID) a ON a.id = b.id) AS sd LEFT JOIN a21iot02.sensor AS s ON s.id = sd.sensorID;";
